...
public final class DAOImpl implements DAO {
    ...
    public String getHashSalt() {
        return (String) constants.stream()
                                 .filter(
                                         c -> "hash_salt".equals(
                                                 c.getName()
                                         )
    ...
    public int getTokenLifetime() {
        return (Integer) constants.stream()
                .filter(
                        c -> "token_lifetime".equals(
                                c.getName()
                        )
                ).findFirst()
                .get()
    ...
    protected static class VehicleCountByInsureeAndModelImpl implements VehicleCountByInsureeAndModel {

        private String insureeFullName;

        private String makeModelYearName;

        private long   count;

        public VehicleCountByInsureeAndModelImpl(String insureeFullName, String makeModelYearName, long count) {
            this.insureeFullName = insureeFullName;
            this.makeModelYearName = makeModelYearName;
            this.count = count;
        }

        @Override
        public String getInsureeFullname() {
            return insureeFullName;
        }

        @Override
        public String getMakeModelYearName() {
            return makeModelYearName;
        }
    ...
    public List<VehicleCountByInsureeAndModel> getVehicleCountByInsureeAndModel() {
        ...
        */
        ...
        String stm;
        Query<VehicleCountByInsureeAndModel> query;

        stm = "SELECT NEW " + VehicleCountByInsureeAndModelImpl.class.getName()
            + "(ma.name, ins.names.firstName || ' ' || ins.names.lastName, COUNT(ve.id)) "
            + "FROM Vehicle ve "
            +   "FULL JOIN ve.model mo "
            +   "FULL JOIN mo.make ma "
            +   "FULL JOIN ve.ownershipsInsuree.key ins "
            + "GROUP BY ma.id, mo.id, ins.id";
    ...
    protected void init() {
        ...
        stm = "FROM " + Constant.class.getName();

        query = session.createQuery(stm);

        constants = query.list();
